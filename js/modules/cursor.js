// ============================================
// CUSTOM FLUID CURSOR — Desktop only
// Based on reference cursor.html (tested working)
// ============================================
...
export function initCustomCursor() {
    ...
    function getUniforms(program) {
        let uniforms = [], count = gl.getProgramParameter(program, gl.ACTIVE_UNIFORMS);
    ...
    function initFramebuffers() {
        ...
        gl.disable(gl.BLEND);
        dye      = dye == null ? createDoubleFBO(dyeRes.width, dyeRes.height, rgba.internalFormat, rgba.format, texType, filtering) : resizeDoubleFBO(dye, dyeRes.width, dyeRes.height, rgba.internalFormat, rgba.format, texType, filtering);
        velocity = velocity == null ? createDoubleFBO(simRes.width, simRes.height, rg.internalFormat, rg.format, texType, filtering) : resizeDoubleFBO(velocity, simRes.width, simRes.height, rg.internalFormat, rg.format, texType, filtering);
    ...
    // Color F0F0F0 del sistema de diseño, atenuado para que sea sutil
    function generateColor() {
        const base = 0xF0 / 255;
        const shade = Math.random();
        const brightness = 0.15;
        return { r: base * shade * brightness, g: base * shade * brightness, b: base * shade * brightness };
    }
