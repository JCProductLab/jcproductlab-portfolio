...
2. **El título entraba con movimiento vertical, no solo horizontal.** Pedido explícito: el título "El impacto en los usuarios" debe entrar EXACTAMENTE igual que "El impacto en el negocio" de Fase 2 — solo derecha→izquierda, sin componente vertical. Causa: el rango de entrada propio del título (0.00–0.25) se solapaba casi por completo con el revelado del contenedor `.rs-usuarios` (0.00–0.20, corrección #1 de arriba), así que el título heredaba el `translateY` del padre mientras ya era visible — a diferencia de Fase 2, donde el título tiene un `RS_TITLE_DELAY` que lo mantiene casi invisible durante la ventana de revelado del contenedor. Fix: se cancela el `translateY` heredado con un `y: -containerRevealY` propio del título (los transforms puros se suman en pantalla, así que el neto es 0) — el título queda con posición vertical fija en todo momento, solo `x`/`opacity` animan.
