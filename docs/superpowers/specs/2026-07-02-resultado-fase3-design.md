...
## Correcciones encontradas al verificar en vivo en el navegador

Estas dos no eran visibles en la revisión estática del código (spec/plan) — solo aparecieron al probar el scroll real:

1. **Faltaba el revelado del contenedor `.rs-usuarios`.** Ni el plan original ni las correcciones anteriores incluían, dentro de `onUpdate`, la transición `y: 100vh → 0` de `.rs-usuarios` — a diferencia de `.rs-mosaico` (Fase 1) y `.rs-metricas` (Fase 2), que sí la tienen. Sin ese bloque, la sección quedaba oculta (su reposo CSS) durante todo el scroll hacia adelante; confirmado en vivo: pantalla en negro sólido durante toda la Fase 3. Se agregó el mismo patrón de "revelado 0.00→0.20" ya usado en las fases anteriores.
2. **El título entraba con movimiento vertical, no solo horizontal.** Pedido explícito: el título "El impacto en los usuarios" debe entrar EXACTAMENTE igual que "El impacto en el negocio" de Fase 2 — solo derecha→izquierda. Primer intento (descartado): cancelar el `translateY` heredado del contenedor con un `y: -containerRevealY` propio del título — funcionaba numéricamente, pero era un mecanismo que Fase 2 no tiene (un parche, no una copia). Fix definitivo: se clona la mecánica exacta de Fase 2 — mismos valores `RS_TITLE_DELAY = 0.10` / `RS_TITLE_SPAN = 0.20` (declarados como `RS_USUARIOS_TITLE_DELAY`/`RS_USUARIOS_TITLE_SPAN` para no chocar con los ya declarados por Fase 2 en el mismo scope de archivo), misma ease `power2.out`, solo `x`/`opacity`, sin ningún `y`. Verificado en vivo comparando ambos títulos punto por punto en progresos equivalentes (0.05 a 0.35): las curvas de `top`/`left` son prácticamente idénticas entre Fase 2 y Fase 3 (mismo drift vertical mientras el contenedor se revela y el título aún es casi transparente, mismo asentamiento horizontal después).
3. **El párrafo podía desbordar el borde inferior en viewports de poca altura.** El `top` dinámico (medido desde la imagen) resuelve el solape con la imagen, pero no garantiza que el párrafo quepa completo antes del borde inferior — confirmado en vivo con una altura efectiva de 907px (desbordaba ~21px). Se agregó una segunda cota: `top final = min(justo-debajo-de-la-imagen, deja-buffer-hasta-el-borde-inferior)`. En viewports muy comprimidos esto puede significar un ligero solape con la imagen en su lugar — limitación estructural del patrón (título+imagen+párrafo en una sección `100vh` fija), no introducida por esta fase, y fuera de alcance resolver aquí (solo desktop, sin rediseñar el patrón de fases anteriores).
