...
- **El título entraba con movimiento vertical, no solo horizontal.** Pedido explícito del usuario: debe entrar EXACTAMENTE igual que "El impacto en el negocio" de Fase 2. Primer intento (descartado por el usuario, correctamente: era un parche): cancelar el `translateY` heredado del contenedor con un `y: -containerRevealY` propio del título — funcionaba, pero Fase 2 no usa ese mecanismo, así que no era una copia real. Fix definitivo: se clona la mecánica exacta de Fase 2 — mismos valores de delay (0.10) y span (0.20) vía constantes propias `RS_USUARIOS_TITLE_DELAY`/`RS_USUARIOS_TITLE_SPAN` (no se reusan `RS_TITLE_DELAY`/`RS_TITLE_SPAN` de Fase 2 porque ya están declaradas en el mismo scope de archivo — redeclararlas sería un error), misma ease, solo `x`/`opacity`, sin `y`. Verificado en vivo: comparando ambos títulos punto por punto (progresos 0.05 a 0.35 dentro de cada fase), las curvas de `top`/`left` son prácticamente idénticas.
